...
**Architecture:** A pure `normalize(svg, { name }) → { svg, findings }` string transform with no
...
`normalize`; judgment calls come back as findings and a human resolves them. `scripts/utils/
...
- **No new dependencies**, not even devDependencies. `normalize.js` imports nothing at all.
...
| File | Responsibility |
|---|---|
| `.agents/skills/svg-prep/normalize.js` | Pure transform. Tokenize → rewrite → serialize, plus findings. No imports. |
| `.agents/skills/svg-prep/prep.js` | CLI. Walks paths, reads, calls `normalize`, writes in place, prints the report, sets the exit code. Only file using `node:fs`. |
| `.agents/skills/svg-prep/SKILL.md` | When to invoke, how to work the findings, the report format. |
| `test/skills/svg-prep.test.js` | Tests `normalize` and its exported internals in real Chrome. |
...
### The `normalize` contract, fixed here so every task agrees
 ...
 *        flatten convert paint even when the file has more than one color
...
| Code | Level | Meaning |
|---|---|---|
| `no-viewbox` | error | No `viewBox` and no `width`/`height` to synthesize one from |
| `multi-color` | error | More than one distinct paint value, and neither `--flatten` nor `--palette`. Downgrades to `warn` under `--palette`, because a hex palette cannot follow the color scheme |
...
- **web-test-runner serves a dot-directory.** `test/skills/svg-prep.test.js` imports
  `../../.agents/skills/svg-prep/normalize.js` directly. No symlink, and `normalize.js` did not
...
| # | Task | Outcome |
|---|---|---|
| 1 | Tokenizer and serializer | `parse` / `serialize` over a flat token list. Also proved the two wiring assumptions above. |
| 2 | The root element contract | `decorateRoot`: `id="icon"`, `xmlns`, `viewBox` synthesized from `width`/`height`. |
...
| 4 | Paint conversion and color findings | Kept in full below. |
...
| 10 | The atomic swap | `icons.js` → `svg.js`, the fixed `#icon` fragment, and all six files re-normalized together. `helix-color.svg` took `--palette`; `more.svg`'s duplicate path was deleted by hand. |
...
### Task 4: Paint conversion and color findings

**Files:**
- Modify: `.agents/skills/svg-prep/normalize.js`
- Modify: `test/skills/svg-prep.test.js`

**Interfaces:**
- Consumes: token list post-`resolveClasses`.
- Produces: findings `multi-color` and `gradient-paint`.
...
single-color file converts without `--flatten`. Update the assertion to
...
Call it after `resolveClasses`. When `findings` gains an `error`, `normalize` returns the original
...
```bash
git add .agents/skills/svg-prep/normalize.js test/skills/svg-prep.test.js
git commit -m "Convert icon paint to currentColor and refuse multi-color files"
```
...
1. **It used `continue` throughout.** `no-continue` is an error under
   ...
   it, rewrote `serialize` as an if/else chain, and the rest of the plan was brought into line.
...
3. **The `-color` branch it described could not work.** Keeping a palette left the file
   multi-color, `normalize` refuses to write a multi-color file, and a refused file gets none of
   the structural rewrites either — so a color variant would have kept its hex *and* never gained
...
4. **`resolveClasses` dropped `<defs>` wholesale.** Gradients are a warning rather than a refusal,
   so a file can legitimately reach serialization with gradient definitions inside `<defs>` — and
...
5. **The `getSvg` it pointed at omitted `xmlns`.** `parseFromString(str, 'image/svg+xml')` is
   strict XML: without a namespace declaration the root is a generic `Element` that serializes as
...
Two things generalize from that list.
