...
| Two hardcoded pinks in an Illustrator `<style>` block, so the icon ignores the color scheme | same file |
...
of what an icon in this project is, so every new one is normalized by hand, differently, and only
...
and capitals may or may not be allowed, and whatever sanitizes `:2_Name-Of--Icon:` has to be found
...
survive the normalizer, follow from it rather than motivate it.

What the fixed fragment does **not** remove is the filename itself: the icon resolves by name, and
an authored `:name:` sanitizes to lowercase alphanumerics and hyphens, so the file has to match.
...
by which an icon can respond to anything — the link it sits in, the section around it, the color
scheme. A fill of `#1a1a1a` is not a style choice that can be overridden later; it is permanent.

So the normalizer converts every paint value to `currentColor`, preserving `none`, and any icon it
cannot convert becomes a reported finding rather than a silent pass.

### Multi-color icons stop and ask

More than one distinct paint value is a judgment call the script does not make. It reports the
colors, names any `#fff` sitting over a color as a probable knockout, and offers three branches:

- flatten everything to `currentColor`;
- keep the palette as hex, which is the existing `helix` / `helix-color` convention. **Every other
  rewrite still happens** — the root id, the editor class styles, the stripping — because a color
...
warning naming the colors left as authored, because an edit silently *not* made is the same defect
as a finding silently becoming an edit.

The `-color` branch costs the color scheme: a hex palette will not follow light and dark, because
...
lives elsewhere and was never normalized, so it keeps its own root id. That pair is what makes the
...
generic `Element` that serializes as `<svg>` and never paints. See
...
mechanism, which is why the normalizer strips `<title>` and `<desc>` rather than preserving them.
...
the same change owns, with no behavioral content.
...
### The transform is a pure function; the script makes no judgments

`normalize(svg, { name }) → { svg, findings }` imports nothing, so it runs in Node and in the
...
The split is the point: mechanical rewrites happen, judgment calls become findings, and **a
finding never silently becomes an edit**. The agent works the findings with a human.

Rejected: svgo. It would replace most of `normalize.js` with configuration and is far better tested
...
.agents/skills/svg-prep/
  SKILL.md            when to use it, the judgment calls, the report format
  normalize.js        pure, no imports
  prep.js             node shim: read → normalize → write in place
...
| `viewBox` | required; synthesized from `width`/`height` when absent, and when neither exists the file is left untouched and the run exits non-zero |
...
- **More than one distinct color** — the branch above, with probable knockouts named.
...
- **Filename not lower kebab-case** — the icon resolves by name, and an authored `:name:` sanitizes
...
- **`img/logos/`.** A parallel convention `loadIcons` never touches, and logos keep their brand hex
  by definition. The skill will normalize one if pointed at it — it is path-driven — but the logos
  are not in scope and are not being re-run.
- **A themeable second color.** Not pursued: one icon needs it and the `-color` file answers it.
  ...
  scheme and still fall back to the brand color. That is a real option for later, not a platform
...
- **Namespacing internal ids.** `svg-prep` strips every id nothing references, so a normalized icon
